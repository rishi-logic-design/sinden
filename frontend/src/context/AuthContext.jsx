...
export const AuthProvider = ({
  ...
  const fetchProfileFromApi = async (providedToken) => {
    try {
      const resp = await fetch("http://localhost:5001/api/auth/me", {
        method: "GET",
        headers: {
      ...
      // Expecting { user: { id, fullName, email, role, ... } } or user object directly
  ...
  // Try to extract role/user from token payload; fallback to /me
    ...
    try {
      ...
      if (!response.ok && response.status === 400) {
        ...
        if (storedRefreshToken) {
          if (!silent) console.log("[Auth] Trying with stored refresh token...");
          response = await fetch(`${baseUrl}/refresh`, {
            method: "POST",
      ...
      if (response.ok && data) {
        ...
        if (newToken) {
          ...
          if (!silent) console.log("[Auth] Refresh succeeded but no token in response");
          return "refreshed";
        }
      } else {
  ...
  const login = async (newToken, refreshTokenValue = null, userFromServer = null) => {
